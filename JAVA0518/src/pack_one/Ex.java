...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * - 자바의 클래스 파일을 모아놓은 공간
		 *   => 서로 다른 패키지에는 같은 이름의 클래스가 각각 따로 존재 할 수 있음
		 *      (같은 이름의 파일이 서로 다른 폴더에 존재할 수 있는 것과 동일)
		 *      = 같은 이름의 클래스가 같은 패키지에 위치할 수 없음!
		 * - 자바에서 패키지를 생성하며, 실제 폴더 구조로 된 패키지가 생성됨
		 ...
		 *      클래스를 선택하는 창이 표시되므로 클래스를 선택하면 됨!
		 *      
		 * < import 키워드 사용 기본 문법 >
		 * 1. import 패키지명.클래스명;
		 * 2. import 패키지명.*;
		...
		// java.lnag 패키지는 생략 가능한 패키지 이름이므로 클래스명만 지정 가능
		String str2;
		
		// java.lang 패키지 이외의 나머지 패키지명을 생략할 경우
//		Random r; // 컴파일 에러 발생!
