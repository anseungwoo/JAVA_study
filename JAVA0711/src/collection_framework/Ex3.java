...
//		list.add(3.14); // 제네릭 타입 고정으로 인해 정수 외에 추가 불가
		...
		// => 단, 제레릭 타입을 Integer로 지정했으므로 리턴 타입이 Integer
		System.out.println("1번 인덱스에 저장된 요소 : " + list.get(1));
		// 배열과 마찬가지로 존재하지 않은 인덱스를 지정할 경우
		// => java.lang.IndexOutOfBoundsException
//		System.out.println("1번 인덱스에 저장된 요소 : " + list.get(4));
...
//		System.out.println("list 객체에 저장된 모든 요소 : " + list);
		...
		// add(int index, Object o) : 특정 인덱스에 해당 요소(o) 추가
		// => 기존 데이터는 밀려남
		list.add(1, 5); // 1번 인덱스에 5를 추가(기존 1번 데이터부터 뒤로 밀림)
		System.out.println("list 객체에 저장된 모든 요소 : " + list);
		...
		// => beginIndex 부터 endInded - 1 까지 부분 리스트 추출
