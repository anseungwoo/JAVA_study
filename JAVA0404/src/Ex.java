...
//		System.out.println("변환 전 a1(int) : " + a1 + ", a2(short) : " + a2);
		...
		a1 = a2; // 컴파일러에 의해서 자동으로 형변환 일어남
		// => 변환 후에도 원본 데이터는 아무런 영향이 없음!
...
//		b2 = b1; // 오류 발생! (Type mismatch: cannot convert from int to short)
		// => 큰 데이터타입을 작은 데이터타입으로 이동할 때 자동 형변환 일어나지 않음!
		//    넘침(Overflow)의 위험성 때문에 자동으로 변환해 주지 않음
		// => 개발자가 강제로 형변환 연산자를 사용하여 명시적 형변환을 수행해야 함
...
//		short c2 = c1; // 오류 발생! 강제 형변환 필수!
		...
		//    명시적 형변환 후에도 Overflow가 발생하지 않으므로 원본 데이터는 유지됨
		...
		// => double 타입이 int 타입보다 크므로 컴파일러에 의해서 자동으로 변환됨.
...
//		e1 = e2; // 오류 발생!
		// => int 형 타입이 double 타입보다 작으므로 자동으로 변환이 일어나지 않음!
		e1 = (int)e2;
		...
		long l = 100L;
		float f = l; // long -> float 변환 시 자동 형변환 가능! ( long < float)
		
//		l = f; // 오류발생! float -> long 변환 시 자동 형변환 불가!
