...
public class Ex3 {
	...
	public static void main(String[] args) {
		 ...
		 *   자신이 직접 처리하지 않고, 메서드를 호출한 곳으로 예외 위임 가능
		 * - throws 키워드를 사용
		...
		// 팀장으로부터 위임받았을 때, 현재 메서드가 사장(main()) 일 경우
...
//		}
		...
		// => ArithmeticException과 NullPointerException, ClassNotFoundException 명시하거나
		// => RuntimeException, ClassNotFoundException 으로 명시하거나
	...
	public static void 대리() throws RuntimeException, ClassNotFoundException {
		// 사원1, 사원2 로부터 3개의 예외를 위임받아 처리할 때
...
//		}
		...
		사원(); // ArithmeticException, NullPointerException => RuntimeException
	...
	public static void 사원() throws ArithmeticException, NullPointerException {
		...
		// 예외 처리 방법 1. 예외가 발생한 곳에서 try ~ catch 로 직접 예외 처리
//		try {
...
//		} 
		...
		// 예외 처리 방법 2. 메서드를 호출한 곳으로 throws 키워드로 예외를 위임
		// => 현재 사원() 메서드를 호출한 곳(대리())으로 예외를 던짐(throws)
		// => 사원() 메서드 선언부 마지막에 throws 키워드를 쓰고
		//    throws 뒤에 예외 클래스를 1개 또는 복수개 명시
	...
	public static void 사원2() throws ClassNotFoundException {
		...
		Class.forName(""); // ClassNotFoundException 발생하는 코드
		// => Compile Checked Exception (일반 예외)
		// => 예외 처리가 필수!! (오류로 표시됨)
