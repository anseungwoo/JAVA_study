...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 * - 보통, 생성자를 정의하여 슈퍼클래스에 예외메시지만 초기화
		 */
...
//			Exception e = new Exception("점수 입력 오류 - " + score);
			...
			// thorw 키워드 뒤에 예외 객체를 지정하여 해당 예외를 발생시킴
//			throw e;
			...
			System.out.println("입력받은 점수는 " + score + "점 입니다.");
		}
		
	} // printScore() 메서드 끝
	
	public static void userExceptionMethod(int score) throws InvalidSocreException {
		if(score < 0 || score > 100) {
			// Exception 클래스 대신 사용자 정의 예외클래스인
			// InvalidSocreException 클래스를 사용하여 예외 발생시킬 수 있음
			throw new InvalidSocreException("점수 입력 오류 - " + score);
		}
	}
...
class InvalidSocreException extends Exception {
	
	// 생성자를 정의하여 예외 메시지를 슈퍼클래스의 생성자에 전달
	public InvalidSocreException(String messgae) {
		super(messgae);
	}
}
