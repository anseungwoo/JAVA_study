...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 1. 선행연산자(전위연산자)
		 * - 피연산자 앞(좌측)에 위치하며 값을 1만큼 증가 또는 감소시킴
		 *   이 때, 다른 연산과 결합하여 사용하는 경우
		 *   먼저 피연산자의 값을 1만큼 증가 또는 감소시킨 후 다른 연산에 참여함
		 *   ex) ++a, --a
		 * 
		 * 2. 후행연산자(후위연산자)
		 * - 피 연산자 뒤(우측)에 위치하며 값을 1만큼 증까 또는 감소시킴
		 *   이 때, 다른 연산과 결합하여 사용하는 경우
		...
		// byte형 변수 a 의 값을 1만큼 증가시키는 방법		
		byte a = 10;
		
		// 1. 일반적인 산술연산자와 대입연산를 사용한 방법
//		a = a + 1; // 컴파일에러 발생! (byte + int = int 이므로 저장 불가!)
		...
		//    대입연산자를 통해서 x1 값을 y1에 대입(저장)
		// => 따라서, x1 값이 5 -> 6으로 증가한 후 y1에 저장된 값 6이 전달됨.
		...
		// => 따라서 x2 값 5가 먼저 y2에 저장된 후, x2의 값이 5 -> 6으로 증가
		...
		x2 = 5;
		System.out.println(++x2); // x2 값을 먼저 1만큼 증가시킨 후 출력(6 출력)
		// => 출력문에 x2 값을 전달하기 전 먼저 증감연산자가 사용됨
		
		System.out.println(x2++); // x2 값을 먼저 출력 한 후 x2값을 1증가(6출력)
		// => 출력문에 x2 값을 먼저 전달한 후 증감연산자가 사용됨
		
		// 위의 과정에서 6이 출력된 후 6 -> 7로 증가되었으므로
