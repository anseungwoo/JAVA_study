...
public class Ex2 {
	...
	public static void main(String[] args) {
		/*
		 * 반복문을 사용하여 컬렉션의 요소에 반복 접근하는 방법(공통방법)
		...
		// 1) 제네릭 타입 <E> 이 지정되지 않은 Set 객체는 Object 타입으로 지정되므로
		...
		//    모든 데이터가 Integer 타입으로 저장되므로 int(Integer) 필요
		for(int num : set2) {
		 ...
		 * 3. 반복문 내에서 Iterator 객체의 next() 메서드를 통해 데이터를 가져오기
		 */
		
		Iterator ite = set.iterator(); // Iterator 객체 얻어오기
		while(ite.hasNext()) { // 다음 요소가 존재할 동안 반복(true)
