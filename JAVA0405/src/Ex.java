...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *          ex2) long + float = (float)long + float = float + flaot = float
		 */
...
//		char ch2 = ch + 2; // 오류발생! char + int = (int)char + int이므로
		...
		System.out.println(ch + 2); // ch + 2 = int이므로 문자 'C' 대신 67 출력
...
//		int i2 = i + l; // 오류발생! int + long = long 이므로 int 타입 변수에 저장 불가!
...
//		System.out.println(d1);
		...
		// 리터럴끼리 연산시에는 자동 형변환이 일어나지 않음!
		byte b4 = 10 + 20; // byte 타입 범위 내 정수는 byte 타입 그대로 연산
		System.out.println(b4);
		
		// 단! 리터럴끼리의 연산이라도 표현 가능한 범위를 초과하면 오류 발생!
//		byte b5 = 10 + 200; // byte 범위는 -128 ~ +127 의 범위를 초과하므로 int 로 취급됨
