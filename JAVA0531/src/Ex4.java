...
//		int iNum = 10;
//		long lNum = iNum; // 자동 형변환(작은타입 -> 큰타입)
		
		// 슈퍼클래스타입 Parent 의 레퍼런스 변수 선언
		Parent3 p;
		// 슈퍼클래스타입 변수 p에 서브클래스타입 Child 인스턴스 주소를 전달
		p = c; // 묵시적 형변환(업캐스팅)
...
//		p.childPrn(); // 서브클래스에서 정의한 메서드(접근 불가) => 오류 발생!
		...
		// 슈퍼클래스 Parent 타입 변수 p2 로 접근 가능한 메서드 : 1개
		p2.parentPrn();
		
		// 서브클래스 Child 타입 변수 c2 선언
		Child3 c2;
		// 서브클래스타입 변수 c2 에 슈퍼클래스 타입 인스턴스 전달
//		c2 = p2; // 컴파일에러 발생! 
...
//		c2 = (Child)p2; // Parent -> Child 타입으로 강제 형변환 = 다운캐스팅
		...
		// 오류가 발생하는 이유
		// Child 타입 변수 c2 로 접근 가능한 메서드 : 2개 
...
//		c2.childPrn(); // 실제 Parent 인스턴스에 존재하지 않는 메서드
		...
		// 기본적으로 자바에서 다운캐스팅은 허용되지 않음
		System.out.println("=======================================");
		
		// 다운캐스팅이 가능한 경우
		// Child 타입 인스턴스를 Parent 타입 변수 p3 에 전달 = 업캐스팅
		Parent3 p3 = new Child3(); // 자동(묵시적) 형변환 일어남
		// 업캐스팅에 의해 참조 영역이 축소되므로 접근 가능한 메서드 : 1개
		p3.parentPrn(); // 상속된(공통) 메서드
//		p3.childPrn(); // 상속되지 않은(서브클래스에서 정의한) 메서드 호출 불가!
		
		// 업캐스팅 된 Parent 타입 p3 의 인스턴스를 Child 타입 변수 c3 에 전달
//		Child c3 = p3; // 컴파일에러 발생! 명시적 형변환 필수!
		Child3 c3 = (Child3)p3; // 다운캐스팅 수행
