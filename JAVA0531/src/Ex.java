...
class FinalMember {
	...
	public void change() {
		//		finalMemeber = 99; //final키워드가 붙은 변수(=상수)는 값 변경 불가
		// 기존 값 변경은 불가능하지만 사용은 가능함
		System.out.println("finalMember = " + finalMemeber);
	}
	final int finalLocalVar = 20; // 로컬변수에도 final 키워드 사용 가능
	//	finalLocalVar = 99; // 값 변경 불가

	// 기존 값 변경은 불가능하지만 사용은 가능함
	public int getFinalMemeber() {
		return finalMemeber;
	}
...
final class FinalClass { //final클래스 = 상속 불가
	int num = 10;
