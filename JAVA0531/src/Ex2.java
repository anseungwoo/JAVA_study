...
//		super("피터파커", 16);
		// => 슈퍼클래스의 Person(String, int) 생성자를 호출하여 데이터 전달함
//		isSpiderMode = true;
		
		// 슈퍼클래스의 생성자를 호출하지 않고 자신의 다른 생성자를 호출하여
		// 해당 생성자 내에서 다시 슈퍼클래스의 생성자를 호출해도 된다!
		this("피터파커", 16, true); // 초기화 코드 중복 제거
		System.out.println("SpiderMan() 생성자 호출됨!");
		
		// 생성자 super() 는 생성자 this() 와 마찬가지로
		// 생성자 내에서 가장 먼저 실행되어야 한다!
//		super("피터파커", 16); // 컴파일에러 발생! 
	}
	
	// 이름(name), 나이(age), 스파이더맨모드(isSpiderMode)를 전달받기
	public Student(String name, int age, boolean isStudyMode) {
		// SpiderMan 클래스 생성자에서 모든 멤버변수를 직접 초기화할 경우
//		this.name = name;
//		this.age = age;
//		this.isStudyMode = isStudyMode;
		...
		// 슈퍼클래스의 생성자에서 수행하고, 서브클래스의 멤버만 초기화 직접 수행
		// => 이 때, 슈퍼클래스의 파라미터 생성자를 호출하여 대신 초기화 수행
