...
// => 인터페이스끼리의 상속은 implements가 아닌 extends 사용
// => 인터페이스끼리의 상속은 다중 상속도 가능
...
//    반드시 인터페이스가 가진 추상메서드를 오버라이딩하여 구현해야한다!
// => 또한, 복수개의 인터페이스를 상속받을 수 있다!
class 고래상어 implements 고래, 상어 {
	// 인터페이스 내에서 슈퍼클래스(인터페이스) 모든 메서드가 추상메서드이므로
...
//	// 분명하지 않기 때문에 자바에서는 다중 상속은 금지되어 있음
//}
