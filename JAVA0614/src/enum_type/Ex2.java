...
public class Ex2 {
	...
	public static void main(String[] args) {
		/*
		 * enum 상수 연산 및 메서드
		 * - enum 상수 값은 동등 비교연산자 (==) 로 일치 여부 비교 가능하며,
		 *   switch ~ case 문에서도 enum 타입 비교가 가능함
		 * - enum 타입 상수에는 각각 ordinal(순서번호) 값이 자동으로 부여됨
		 *   => 0부터 부여됨
		...
		switch(today) {
			//case 문을 사용하여 enum 상수를 지정하여 비교 가능
		    // => 주의! enum타입명.상수가 아닌 그냥 상수 자체를 비교!
//			case EnumWeek.MONDAY; // 컴파일에러 발생!
		...
		if(today.ordinal() < 4) {
			...
			System.out.println("오늘은 금요일! 그래도 내일 쉰다!");
		} else {
