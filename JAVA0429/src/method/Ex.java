...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * - 반드시 호출되어야만 사용 가능
		 *   => 호출 당하는 메서드 : Worker 메서드
		 *      호출하는 메서드 : Caller 메서드
		 *   => main() 메서드는 자바에서 유일하게 프로그램 시작 시 자동으로 호출되는 메서드
		 *      (main() 메서드를 프로그램 시작점이라고 함)
		 * - 메서드 호출 시 전달하는 값을 전달인자(Argument) 라고 하며
		 *   메서드에서 전달되는 값(전달인자)을 저장하는 변수를 매개변수(Parameter) 라고 함
		...
		sister_1(); // sister_1() 메서드 호출하는 코드
		// => 코드의 흐름이 sister_1() 메서드 정의한 곳으로 바뀜
		// => sister_1() 메서드 실행이 끝난 후 다시 코드의 흐름이 호출한 곳으로 복귀
