public class Ex2 {
	...
	public static void main(String[] args) {
		...
		/*
		 * instanceof 연산자 - 좌변의 객체(참조변수)가 우변 클래스의 타입인지 판별하는 연산자 - 판별 결과가 true 이면 형변환이
		 * 가능한 관계(업캐스팅 or 다운캐스팅) 판별 결과가 false 이면 절대로 형변환 불가 - 우변의 클래스 타입과 같거나 우변 클래스 타입의
		 * 자식 타입 모두 가능 - A is a B = A instanceof B - 판별 결과를 boolean 타입 변수에 저장하거나 if문에
		 * 판별하는 문장을 직접 사용 가능 < 기본 문법 > if(A instanceof B) { // 형변환이 가능한 관계이므로 변환 수행 }
		 * else { // 절대로 형변환이 불가능한 관계이므로 변환 수행 X }
		 */

		SmartPhone sp = new SmartPhone("갤럭시", "010-1111-1111", "안드로이드");
		sp.call();
		sp.sms();
		sp.kakaoTalk();
...
//			hp.kakaoTalk(); //SmartPhone의 기능은 포기해야함
			// => 업캐스팅할 경우 참조영역이 축소됨
		} else {
		...
		// 다운캐스팅 가능 대상 확인
		// 슈퍼클래스 타입 인스턴스 생성
		HandPhone hp = new HandPhone("애니콜", "010-1234-5678");
		...
		HandPhone hp2 = new SmartPhone("갤럭시", "010-1111-1111", "안드로이드");
