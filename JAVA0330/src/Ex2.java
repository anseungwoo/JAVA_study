...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 *   ex) 'A'는 정수 65로 처리됨.
		 * - 유니코드 표현 방법을 사용
		 *   => Unicode(유니코드) : 전 세계의 모든 문자를 표현하기 위해 만든 체계
		 *   => 아스키코드를 기반으로 해서 나머지 문자를 추가하여 만든 코드 체계
		 *      아스키코드(ASCII, 7bit) : 미국 표준 문자 코드
		 *      'A' = 65, 'a' = 97, '0' = 48
		 *   => 10진수로 된 코드값을 사용하지만, 입력 시 16진수 코드 4자리 사용
		 *      '\'기호와 u를 쓰고 16진수 XXXX 형태의 유니코드값 표현 방법 사용
		 *      ex) \u0000
		 */
...
//		ch = ''; // 반드시 1개의 문자를 포함해야 함
		 ...
		 *   0개 이상의 문자열을 큰 따옴표("")로 둘러싸서 표현함.
		 *   => 아무 문자도 포함하지 않는 "" 문자열을 사용할 수 있고
		 *      이를 널스트링(null string)이라고 함.
		 */
