...
//		b = 128; // 오류 발생! byte 타입에 저장 가능한 정수 -128 ~ +127까지
		...
		// short 타입 변수 s를 선언하고 정수값 128을 저장
		short s = 128;
...
//		i = 2147483648; // 오류 발생
		...
		//	  최대치가 21487483647 까지만 표현이 가능
		// => int 범위를 벗어나는 리터럴은 반드시 접미사 'L' 붙여서
		//    long 타입으로 표기해야 한다!
//		i = 2147483648L; // 오류 발생! long 타입 정수는 int 타입 변수에 저장 X
		...
		// int 타입보다 크지 않은 정수도 접미사 L을 붙이면 long 타입으로 취급됨
//		i = 5L; // 오류 발생! 정수 5는 int 범위에 포함되지만, L 붙이면 저장 불가!
		
		// 실수 데이터타입(float, double)
...
//		bool = 0; // 오류 발생
		...
		int num = 10; //10진수 10 저장
		System.out.println(num);
		
		// 2. 2진수 : 0 또는 1 숫자만을 사용하고, 접두사 0b 붙임
		num = 0b1010;
		System.out.println(num);
		// 주의! 0 또는 1 이외의 값 사용 불가!
//		num = 0b1012; // 오류 발생! 2 사용 불가
		
		// 3. 8진수 : 0 ~ 7 사이의 숫자 사용하며, 접두사 0 붙임
		num = 012;
		System.out.println(num);
