...
//		sc.instance = null;
		...
		// => 이 때, getInstance() 메서드 static 메서드이므로
		//    클래스명.메서드() 형태로 호출 가능
		...
		// 리턴받은 인스턴스는 참조변수(sc2)를 통해 접근 가능
		System.out.println("sc.str : " + sc.str);
		System.out.println("sc2.str : " + sc2.str);
		
		// if 문을 통해 두 인스턴스(sc, sc2)가 같은지 비교
		if(sc == sc2) {
			System.out.println("sc 와 sc2는 같은 인스턴스!");
		} else {
			System.out.println("sc 와 sc2는 다른 인스턴스!");
		}
