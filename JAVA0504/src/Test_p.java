 ...
 * 
 * [ 메서드 정의 ]
 * - 계좌정보, 예금주명, 현재잔고를 출력하는 showAccountInfo() 메서드
 ...
 *      출금될 금액과 출금 후 현재 잔고를 출력
 *      ex) "출금할 금액 		: XXXX원"
 *          "입금 후 현재잔고   : XXXX원"
 *   => 단, 출금 기능을 수행하기 전에 현재잔고와 출금할 금액을 비교하여
 *      현재잔고가 출금할 금액보다 적을 경우 출금을 수행하지 않고 
 *      다음과 같이 출력
 *      ex) "잔고가 부족하여 출금이 불가능합니다."
 *          "출금할 금액 : XXXX원, 현재잔고 : XXXXX원"
...
//		System.out.println("입금 후 현재잔고 : " + balance + "원");
	...
	public int withdraw(int amount) {
		...
		// 출금되는 금액(amount)을 리턴
		// 단, 현재잔고가 출금금액 보다 적을 경우 출금 불가
		if(balance < amount) { // 현재 잔고가 출금 금액보다 적은지 판별(출금불가)
			System.out.println("잔고가 부족하여 출금이 불가능합니다.");
			System.out.println(
					"출금 할 금액 : " + amount + "원, 현재잔고 : " + balance + "원");
			// retrun 타입이 int이므로 반드시 리턴값 필수! => 0원 리턴
//			return 0;
			amount = 0;
